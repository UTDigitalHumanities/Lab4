...
```{r}
tapply(artists$numPieces, artists$Nationality, sum)
```

# Scatter Plots
8.. Make a scatter plot charting number of pieces by artist birth year.
```{r}
plot(artists$Birth.Year, artists$numPieces)
```
...
paintings <- art[art$Classification == "Painting",]

#Then remove all those missing either width or height. Hint: use "!is.na()" to check if something isn't NA
paintings <- paintings[!is.na(paintings$Width..cm.) & !is.na(paintings$Height..cm.),]

#Then, draw the plot:
plot(paintings$Width..cm., paintings$Height..cm.)

```

11. Re-make the same plot without the very large items. 
```{r}
wCutOff <- quantile(paintings$Width..cm., prob=.99)
hCutOff <- quantile(paintings$Height..cm., prob=.99)

plot(paintings[paintings$Width..cm. < wCutOff, "Width..cm."], paintings[paintings$Height..cm. < hCutOff, "Height..cm."])
```

12. Use `lm()` to find a best-fit line.

```{r}
lm(paintings$Width..cm. ~ paintings$Height..cm.)
```
